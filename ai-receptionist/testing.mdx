...
sequenceDiagram
    ...
    participant CRM as CRM / Calendar

    You->>AI: Initiate test call (phone or browser)
    AI->>You: Greeting + conversation
    You->>AI: Ask about services, book appointment, etc.
    AI->>AI: Checks availability, captures lead
    AI->>You: Confirms booking / answers question
    AI-->>Dashboard: Call logged with transcript + summary
    AI-->>Dashboard: Lead created with score + temperature
    AI-->>Dashboard: Appointment created (if booked)
    Dashboard-->>CRM: Auto-sync lead + calendar event
    You->>Dashboard: Review transcript, lead, and booking
```
...
**Check your greeting message.** Go to **Receptionist** and look at your greeting. Make sure the business name is spelled phonetically if it is unusual. For example, if your business is "Beaute Salon", write "Bootay Salon" in the greeting so the AI pronounces it correctly.
</Accordion>
...
<Accordion title="Does testing use my plan minutes?">
Yes. Test calls use your plan minutes just like real calls — the AI runs the full pipeline including voice synthesis and transcription. Budget 2-3 minutes per test call. On the Starter plan, that is well within your monthly allowance.
</Accordion>

<Accordion title="Can I test integrations (CRM, Calendar, Zapier) during a test call?">
Absolutely. Test calls are real calls as far as the system is concerned. If you book an appointment during a test, it will appear on your Google Calendar. If you have HubSpot connected, a lead will be created there. This is the best way to verify your integrations end-to-end.
</Accordion>

<Accordion title="How long until test results appear in the dashboard?">
Call logs, transcripts, leads, and appointments appear within **5 to 10 seconds** of the call ending. Refresh the Calls page if you do not see it immediately. CRM sync and calendar events take an additional 2-3 seconds.
</Accordion>

<Accordion title="Can I test from a different number to simulate a new caller?">
Yes, and we recommend it. Borrow a friend's or colleague's phone and call your AI number. The AI will not recognise the number and will give the full first-time caller experience, including asking for their name and details. This is the best way to test the new customer flow.
</Accordion>
...
<Card title="Test your AI now" icon="arrow-up-right-from-square" href="https://app.closethecall.com/ai-config/test">
  Open the Test AI page to make a test call or start a browser call.
</Card>
